...
class RuleAugmentedEstimator(BaseEstimator):
    ...
    def __init__(self, base_model: BaseEstimator, vectorizer: CountVectorizer, rules: Dict, **base_params):
        ...
        for category, rules in self.rules.items():
            ...
            for rule in rules:
                ...
                if rule[0] == "=":
                    ...
                    train_x = train_x.loc[train_x[category].str.contains(rule[1], case=False)]
    ...
    """
        ...
        # Text Processing
        train_x = train_x.replace(to_replace='<[^<]+?>', value='TAG', regex=True)

        train_x = self.vectorizer.fit_transform(train_x['tag']).toarray()
    ...
    def predict(self, X: pd.DataFrame) -> np.array:
        ...
        if len(p_X.loc[p_X['prediction'].isna()].index != 0):
            ...
            print("Base X: ", base_X)
            base_X = base_X.replace(to_replace='<[^<]+?>', value='TAG', regex=True)
            base_X = self.vectorizer.transform(base_X['tag']).toarray()
